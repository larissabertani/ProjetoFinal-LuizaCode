...
async def create_product(products_collection, product):
    if product['price'] <= 0.01:
        raise HTTPException(status_code=203, detail ="O valor do produto deve ser superior a R$0,01")
    if product['qt_stock'] <= 0:
        raise HTTPException(status_code=203, detail ="A quantidade do produto deve ser superior a 0")
    has_product = await product_models.get_product_by_code(products_collection, product['code'])
    if has_product:
        raise HTTPException(status_code=202, detail ="Já existe produto cadastrado com este código!")    
...
async def get_product_by_name(products_collection, product_name):
    ...
    if product:
        return product
    raise HTTPException(status_code=404, detail = "Não existe produto com este nome!")
...
async def delete_product(product_collection, carts_collection, product_code: int):    
    ...
    raise HTTPException(status_code= 404, detail = "Não há produto com este código para ser deletado!")
